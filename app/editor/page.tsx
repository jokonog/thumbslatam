...
export default function Editor() {
  ...
  useEffect(() => {
    function calcularEscala() {
      ...
      // Medir el panel real disponible menos padding interno (32px cada lado)
    ...
    // Pequeño delay para que el DOM esté listo al montar
  ...
  }, [isMounted]); // solo al montar — NO depende de p.w/p.h para no reinicializar
  ...
  function aplicarImagenAlCanvas(
    ...
    // Para verticales: escalar por altura para garantizar sobrante horizontal
    // Para horizontales: escalar por la dimensión que deje sobrante en el eje correcto
    let scaleFit: number;
    if (npEsVertical) {
      // Escalar para que la altura cubra el canvas — habrá sobrante en X para arrastrar
      scaleFit = np.h / naturalH;
      // Si no cubre el ancho, usar cover normal
      if (naturalW * scaleFit < np.w) {
    ...
    // Verticales: siempre arrastre X (izquierda/derecha), nunca Y
    // Horizontales: arrastre solo si hay sobrante real
  ...
  async function cambiarPlataforma(val: string) {
    ...
    // Redimensionar el canvas de Fabric correctamente (v6 API)
    canvas.setDimensions({ width: np.w, height: np.h });

    // Reasignar imagen si existe
  ...
  function centrarImagen() {
    ...
    // Usar la misma lógica de escala que aplicarImagenAlCanvas
    let scaleFit: number;
    if (npEsVertical) {
    ...
    // Centrar horizontalmente (para verticales) o en ambos ejes (para horizontales)
    img.set({
      left: -(scaledW - p.w) / 2,
      top: npEsVertical ? -(scaledH - p.h) / 2 : -(scaledH - p.h) / 2,
    });
    canvas.renderAll();
  }
  ...
  async function undo() {
    ...
    const estado = JSON.parse(
      historialRef.current[historialRef.current.length - 1]
    );
    canvas.getObjects().forEach((obj: any) => canvas.remove(obj));
  ...
  // ─── Dimensiones del contenedor visual (escala aplicada) ────────────────────
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          >
            ...
            {orientacionIncompatible && (
              ...
              >
                ⚠️ Fondo {fondoEsVertical ? "vertical" : "horizontal"} — genera
                de nuevo en formato {esVertical ? "vertical" : "horizontal"} para
                mejor resultado.
              </p>
            )}
          </div>
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                padding: "10px",
                borderRadius: "8px",
                background: tema && !cargando ? "#FF4D00" : "#3A3D52",
                border: "none",
                color: "white",
                fontWeight: "700",
                cursor: tema && !cargando ? "pointer" : "not-allowed",
                fontSize: "0.85rem",
              }}
            >
              {cargando ? (
          ...
          >
            ...
            <input
              ...
              accept="image/*"
              onChange={subirFondo}
              style={{ color: "#8B8FA8", fontSize: "0.8rem", width: "100%" }}
            />
          </div>
          ...
          >
            ...
            >
              <label style={{ fontSize: "0.78rem", color: "#8B8FA8" }}>
                Color:
              </label>
              <input
                type="color"
              ...
              />
              <span style={{ fontSize: "0.78rem", color: "#8B8FA8" }}>
                {colorTexto}
              </span>
            </div>
            <button
              ...
              style={{
                ...
                padding: "9px",
                borderRadius: "8px",
                background: texto ? "#FF4D00" : "#3A3D52",
                border: "none",
                color: "white",
                fontWeight: "700",
          ...
          >
            <p style={{ color: "#06D6A0", fontSize: "0.78rem", margin: 0 }}>
              Arrastra el texto para moverlo. Esquinas para cambiar tamaño.
            </p>
          </div>
        </div>
...
{esVertical ? "Arrastra la imagen izquierda/derecha para elegir el encuadre →" : "Arrastra la imagen para elegir el encuadre →"}
          ...
          {/*
            Contenedor exterior: tamaño exacto del canvas escalado.
            overflow:hidden recorta lo que se sale de la vista.
          */}
          <div
            style={{
              width: `${canvasW}px`,
              ...
              overflow: "hidden",
              borderRadius: "8px",
              // Centrar dentro del panel si el panel es más ancho
              margin: "0 auto",
            }}
          >
            {/*
              Contenedor interior: tamaño REAL del canvas (p.w × p.h).
              El scale() lo achica visualmente al tamaño del exterior.
              transformOrigin:"top left" es clave para que no se desplace.
            */}
            <div
              style={{
                position: "absolute",
